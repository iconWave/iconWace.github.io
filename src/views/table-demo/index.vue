<template>
  <div class="node-conatiner">
    ...
    <Alert type="info" message="Mock + fakerjs 数据模拟基本用法，及Table + filter筛选组件组合用法" />
    <Table
      ref="ELRef"
    ...
    <Modal v-bind="modalState" @cancel="handleCancel" @ok="handleSubmit">
      <a-form ref="FormRef" :model="formModel" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol">
        ...
        <a-form-item label="角色:" name="role_id">
          <a-select v-model:value="formModel.role_id" :options="roleOptions" placeholder="请选择角色" />
        </a-form-item>
      </a-form>
    </Modal>
  </div>
...
const tableFilterItems = computed(() => [
  ...
  },
  {
    ...
    search: () => console.log(toRaw(tableFilterModel))
  }
])
</script>
